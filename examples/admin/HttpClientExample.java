...
public class HttpClientExample {
    ...
    public void run() throws Exception {
        ...
        node.put("command", "show");

        /* HTTPS access secured KVLite */
        if (password != null && !password.isEmpty()) {
            runShowTopology(createHttpsConn(commandPath), node);
            return;
        }

        /* HTTP access non-secured KVLite */
        runShowTopology(createHttpConn(commandPath), node);
    }

    private void runShowTopology(HttpURLConnection urlConn, ObjectNode node)
        throws Exception {
        ...
        output.writeBytes(node.toString());
        output.close();
    ...
    private HttpURLConnection createHttpsConn(String commandPath)
        ...
        /* Find login token from the request result */
        final JsonParser parser = JsonUtils.createJsonParser(input);
        final String token =
            parser.readValueAsTree().
                get("returnValue").get("token").asText();
        input.close();
